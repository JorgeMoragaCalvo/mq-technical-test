...
- `UF_VALUE_CLP` como configuración basada en variables de entorno (por defecto `40000`) — sin números mágicos en el código.
...
- `reconcile()` atómico que hace cumplir la invariante *una transferencia no puede asignar más que su monto*; reaplicar sobre el mismo par (movimiento, cobro) actualiza la asignación.
- API REST (router de DRF, reemplaza el obsoleto `/api/tasks/`):
...
- Panel de conciliación: elegir una transferencia → asignar montos a los cobros pendientes → indicador en vivo de capacidad restante que bloquea la sobreasignación antes de enviar.
...
- **UF fija = CLP 40.000**, configurable mediante la variable de entorno `UF_VALUE_CLP` (valor por defecto de la especificación).
...
- **Valor de la UF**: ¿debería ser la UF *oficial diaria* (basada en fecha, por `fecha`/`mes_cobro`) en lugar de una constante fija? ¿Qué fecha rige un pago que cruza de mes?
- **Propiedad del crédito**: ¿quién es dueño de un crédito por sobrepago — el contrato? ¿Se arrastra al cobro del mes siguiente, se reembolsa o se retiene? Actualmente solo lo reportamos.
...
- **Política de redondeo**: ¿es correcto half-up para UF↔CLP, y cuántos decimales para la UF (usamos 2)?
